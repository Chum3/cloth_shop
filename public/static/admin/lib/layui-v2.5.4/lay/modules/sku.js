...
;layui.define(['jquery', 'form'], function (e) {
    ...
    var $ = layui.jquery, form = layui.form, o = {
        ...
        sth: function () {
            var h = '<tr>';
            for (var i = 0; i < this.c.tableTitle.length; i++) {
                h += '<th>' + this.c.tableTitle[i] + '</th>';
            }
            h += '<th>价格</th><th>库存</th><th>货号</th></tr>';
            return h;
        },
        stb: function () {
            var t = this, h = '', na = [];
        ...
        citd: function (i, b) {
            if (b && !this.inArray(i, this.c.itemed)) {
                this.c.itemed.push(i);
                return;
            }
            if (!b && this.c.itemed.indexOf(i) > -1) {
                this.c.itemed.splice(this.c.itemed.indexOf(i), 1);
                return;
            }
        },
        cd: function () {
        ...
        listenInput: function () {
            ...
            $('input[lay-filter]').on('change', function (e) {
                ...
                if (p == 'sku-sku') {
                    for (var k = 0; k < t.c.data.length; k++) t.c.data[k].sku = v + (k + 1);
                }
                if (p == 'sku-price') {
                    for (var k = 0; k < t.c.data.length; k++) t.c.data[k].price = isNaN(parseFloat(v)) ? 0 : parseFloat(v);
                }
                if (p == 'sku-stock') {
                    for (var k = 0; k < t.c.data.length; k++) t.c.data[k].stock = isNaN(parseInt(v)) ? 0 : parseInt(v);
                }
                t.rl();
            })
        },
